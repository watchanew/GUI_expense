...
def Save(event=None):
    ...
    transacid = stamp.strftime('%Y%m%d%H%M%S')
    if expense == '':
        print('No data')
    ...
    try:
        ...
        with open(r'C:\Users\Asus\Desktop\EP-7\savedata2.csv', 'a', encoding='utf-8',
            ...
            data = [transacid, time, expense, price, amount, total]
            fw.writerow(data)
        # Return cursor to E1
        E1.focus()
        update_table()
...
# Tab2
def read_csv():
    with open(r'C:\Users\Asus\Desktop\EP-7\savedata2.csv', newline='', encoding='utf-8') as f:
        fr = csv.reader(f)
        data = list(fr)
    return data
...
# Treeview widget
header = ['Transaction ID','Time','List','Price','Amount','Total']
...
def UpdateCSV():
     with open(r'C:\Users\Asus\Desktop\EP-7\savedata2.csv','w', newline='', encoding='utf-8') as f:
        fw = csv.writer(f)
        #tranform alltransaction to list
        data = list(alltransaction.values())
        fw.writerows(data)
        print('Table was updated')
        update_table()   

#function delete data in csv via treetable 
def DeleteRecord(event=None):
    checking = messagebox.askyesno('Confirm?', 'Do you want to delete this data?')
    print('YES/NO:',checking)
    if checking == True:
        print('delete')
        ...
        transacid = data[0]
        #print(transacid)
        #print(type(transacid))
        del alltransaction[str(transacid)] # delete data in dict
